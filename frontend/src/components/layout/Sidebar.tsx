      ...
      <aside className={cn(
        ...
        <div className="px-4 pt-5 pb-4 border-b border-white/8 shrink-0">
          ...
          {selectedStudy ? (
            <>
              <div className="relative">
                <button
                  ...
                  className="w-full flex items-start gap-2 px-3 py-2.5 bg-white/6 hover:bg-white/10 border border-white/10 rounded-lg transition-colors text-left"
                >
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-gray-400 text-[10px] truncate">{selectedStudy.indication}</p>
                  </div>
                  <ChevronDown className={cn('h-3.5 w-3.5 text-gray-500 mt-1 shrink-0 transition-transform', studyOpen && 'rotate-180')} />
                </button>

                {studyOpen && (
                  <div className="absolute top-full left-0 right-0 mt-1 bg-[#111112] border border-white/12 rounded-lg shadow-2xl z-50 overflow-hidden">
                    {studyList.map(s => (
                      <button
                        key={s.id}
                        onClick={() => { onStudyChange(s); setStudyOpen(false) }}
                        className={cn(
                          'w-full px-3 py-2.5 text-left hover:bg-white/6 transition-colors border-b border-white/6 last:border-0',
                          s.id === selectedStudy.id && 'bg-[#2563EB]/15',
                        )}
                      >
                        <p className="text-white text-xs font-semibold">{s.protocol}</p>
                        <p className="text-gray-400 text-[10px] mt-0.5">{s.indication}</p>
                        <div className="flex items-center gap-1.5 mt-1">
                          <span className={cn('text-[9px] font-bold uppercase tracking-wider px-1.5 py-0.5 rounded',
                            s.locked ? 'bg-emerald-900/60 text-emerald-400' : 'bg-[#2563EB]/20 text-[#60a5fa]'
                          )}>
                            {s.status}
                          </span>
              ...
              <div className="flex items-center gap-3 mt-2.5 px-0.5">
                <span className="text-[10px] text-gray-500">Estimand: <span className="text-[#60a5fa] font-semibold">{selectedStudy.estimand}</span></span>
            ...
            <div className="px-3 py-3 bg-white/4 border border-white/8 rounded-lg">
              <p className="text-[10px] text-gray-500 uppercase tracking-widest font-semibold">No Active Study</p>
              <p className="text-gray-400 text-[10px] mt-1">Open a project from the dashboard to begin.</p>
            </div>
          )}
        </div>
        ...
        {/* ── Literature Search ───────────────────────────────────────────── */}
        {selectedStudy && (
          <div className="px-2.5 pb-2 border-t border-white/8 pt-2 shrink-0">
        ...
        {/* ── User footer ─────────────────────────────────────────────────── */}
        <div className="px-3 pb-3 border-t border-white/8 pt-3 shrink-0 bg-black/20">
          <div className="flex items-center gap-2.5">
            <Avatar className="h-7 w-7 shrink-0">
              <AvatarFallback className="bg-[#2563EB]/30 text-[#60a5fa] text-[10px] font-bold">{initials}</AvatarFallback>
            </Avatar>
            <div className="flex-1 min-w-0">
            ...
            <button onClick={toggleTheme} className="text-gray-600 hover:text-gray-300 p-1 transition-colors" title={isDark ? 'Switch to light mode' : 'Switch to dark mode'}>
              {isDark ? <Sun className="h-3.5 w-3.5" /> : <Moon className="h-3.5 w-3.5" />}
            </button>
            <Link to="/admin/settings" className="text-gray-600 hover:text-gray-300 p-1 transition-colors" title="Settings">
          ...
          <div className="mt-1.5 flex items-center gap-2 px-0.5">
            <Link to="/terms" className="text-[8px] text-gray-600 hover:text-gray-400 transition-colors">Terms</Link>
            <span className="text-[8px] text-gray-700">·</span>
            <Link to="/privacy" className="text-[8px] text-gray-600 hover:text-gray-400 transition-colors">Privacy</Link>
            <span className="text-[8px] text-gray-700">·</span>
            <Link to="/policies/computational-methods" className="text-[8px] text-gray-600 hover:text-gray-400 transition-colors">Policies</Link>
          </div>
        </div>
      </aside>
    </>
