...
import { z } from 'zod'
import { STUDY_PHASES, REGULATORY_AGENCIES } from '../types/regulatoryConfig'
...
export default function EnhancedDashboard() {
  ...
  const fetchProjects = useCallback(async () => {
    ...
    try {
      ...
      // Normalize status to lowercase — seed data and some API paths return uppercase enum names
  ...
  const handleCreate = async (title: string, description: string, researchIntent: string, phase: string, agency: string) => {
    await apiClient.request('/projects', z.any(), {
      method: 'POST',
      body: JSON.stringify({ title, description, research_intent: researchIntent, phase, agency }),
    })
    setModalOpen(false)
    void fetchProjects()
  }
    ...
    <div className="min-h-screen bg-gray-50 p-6 md:p-10">
      ...
      {error && (
        <div className="mb-6 flex items-center gap-2 rounded-lg bg-red-50 border border-red-200 p-4 text-red-700">
          ...
          <button onClick={() => void fetchProjects()} className="shrink-0 px-3 py-1.5 text-xs font-semibold border border-red-300 rounded-lg hover:bg-red-100 transition-colors">
            Retry
          </button>
        </div>
      )}
      ...
      {!loading && filtered.length > 0 && (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filtered.map((project) => (
            <div
              key={project.id}
              className="rounded-xl bg-white shadow hover:shadow-md transition p-6 flex flex-col"
            >
              {/* Title + badge */}
              ...
              <div className="flex items-center gap-2">
                <button
                  onClick={() => navigate(`/projects/${project.id}/study`)}
                  className="flex-1 inline-flex items-center justify-center gap-1 rounded-lg bg-blue-50 text-blue-600 px-3 py-1.5 text-sm font-medium hover:bg-blue-100 transition"
                >
                ...
                {project.status !== 'archived' ? (
                  <button
                    ...
                    className="inline-flex items-center gap-1 rounded-lg bg-gray-100 text-gray-600 px-3 py-1.5 text-sm font-medium hover:bg-gray-200 transition"
                  >
                  ...
                  <>
                    <button
                      ...
                      className="inline-flex items-center gap-1 rounded-lg bg-emerald-50 text-emerald-600 px-3 py-1.5 text-sm font-medium hover:bg-emerald-100 transition"
                    >
...
function CreateProjectModal({
  ...
  onClose: () => void
  onCreate: (title: string, description: string, researchIntent: string, phase: string, agency: string) => Promise<void>
}) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSubmitting(true)
    try {
      await onCreate(title, description, researchIntent, phase, agency)
    } finally {
      setSubmitting(false)
    }
  }
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50" role="dialog" aria-label="Create project">
      <div className="w-full max-w-lg rounded-xl bg-white shadow-xl p-6 mx-4">
        ...
        <form onSubmit={(e) => void handleSubmit(e)} className="space-y-4">
          <div>
            <label htmlFor="proj-title" className="block text-sm font-medium text-gray-700 mb-1">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label htmlFor="proj-phase" className="block text-sm font-medium text-gray-700 mb-1">
                Regulatory Phase <span className="text-red-500">*</span>
              </label>
              <select
                id="proj-phase"
                required
              ...
              >
                ...
                {STUDY_PHASES.map(p => <option key={p.value} value={p.value}>{p.label}</option>)}
              </select>
            </div>
            <div>
              <label htmlFor="proj-agency" className="block text-sm font-medium text-gray-700 mb-1">
              ...
              >
                ...
                {REGULATORY_AGENCIES.map(a => <option key={a.value} value={a.value}>{a.label}</option>)}
              </select>
            </div>
          </div>
          ...
          <div>
            <label htmlFor="proj-intent" className="block text-sm font-medium text-gray-700 mb-1">
              Research Intent
            </label>
            <textarea
              id="proj-intent"
          ...
          <div className="flex justify-end gap-3 pt-2">
            ...
            <button
              type="submit"
              disabled={submitting || !title.trim() || !phase || !agency}
