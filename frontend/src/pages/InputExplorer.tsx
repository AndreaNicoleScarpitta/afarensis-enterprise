...
function missingnessLabel(rate: number): string {
  ...
  if (rate < 0.20) return 'text-amber-600'
  return 'text-red-400'
}
...
function sourceTypeBadgeColor(type: string): string {
  ...
  return map[type] || 'bg-gray-800/40 text-gray-600 border-gray-700/40'
}
        ...
        <div className="flex items-start justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <h1 className="text-xl font-bold text-gray-900">Data Source Inventory & Schema</h1>
          ...
          <div className="text-right">
            ...
            <p className="text-[10px] text-gray-500 mt-0.5">Estimand: <span className="text-[#2563EB] font-semibold">{selectedStudy.estimand}</span></p>
          </div>
        </div>
      </div>
            ...
            <div className="bg-gray-100/80 border border-gray-200 rounded-xl overflow-hidden">
              <div className="px-5 py-4 border-b border-gray-200 flex items-center justify-between">
                <div>
                  ...
                  <p className="text-sm font-bold text-gray-900 mt-0.5">{dataSources.length} Registered Sources</p>
                </div>
                <div className="flex items-center gap-2">
              ...
              {/* Table header */}
              <div className="grid grid-cols-[2fr_1fr_1.2fr_1fr_0.8fr_0.8fr_0.6fr_0.5fr] gap-3 px-5 py-2.5 bg-gray-200/60 text-[10px] font-bold uppercase tracking-widest text-gray-500">
              ...
              {dataSources.map((source: DataSource) => {
                ...
                return (
                  <React.Fragment key={source.id}>
                    <div
                      className={`grid grid-cols-[2fr_1fr_1.2fr_1fr_0.8fr_0.8fr_0.6fr_0.5fr] gap-3 px-5 py-3.5 border-t border-gray-200 hover:bg-gray-200/40 transition-colors cursor-pointer items-center`}
                      onClick={() => setExpandedSource(isExpanded ? null : source.id)}
                    >
                      <div className="flex items-center gap-2 min-w-0">
                        <ChevronRight className={`h-3.5 w-3.5 text-gray-500 shrink-0 transition-transform ${isExpanded ? 'rotate-90' : ''}`} />
                    ...
                    {isExpanded && dv && (
                      <div className="border-t border-gray-200 bg-gray-50 px-8 py-5">
                        <div className="flex items-center gap-2 mb-4">
                          ...
                          <p className="text-sm font-bold text-gray-900">Schema Browser</p>
                        ...
                        <div className="border border-gray-200 rounded-lg overflow-hidden">
                          {/* Schema header */}
                          <div className="grid grid-cols-[1.5fr_0.8fr_0.6fr_0.6fr_1fr_0.8fr_1fr] gap-3 px-4 py-2 bg-gray-200/60 text-[10px] font-bold uppercase tracking-widest text-gray-500">
                          ...
                          {dv.fields.map((field: FieldDefinition, idx: number) => (
                            <div
                              key={field.name}
                              className={`grid grid-cols-[1.5fr_0.8fr_0.6fr_0.6fr_1fr_0.8fr_1fr] gap-3 px-4 py-2.5 border-t border-gray-200 ${idx % 2 === 0 ? 'bg-transparent' : 'bg-gray-100/50'}`}
                            >
                              <div>
                                <p className="text-xs font-mono font-semibold text-gray-900">{field.name}</p>
                                <p className="text-[10px] text-gray-500 truncate">{field.description}</p>
                              </div>
                              <div className="text-[11px] text-gray-600 capitalize">{field.type}</div>
                              ...
                              <div className="text-xs font-mono text-gray-600">{field.distinctValues.toLocaleString()}</div>
                        ...
                        <div className="mt-4 grid grid-cols-4 gap-4">
                          <div className="bg-gray-100/80 border border-gray-200 rounded-lg p-3">
                            ...
                            <p className="text-xs font-mono text-gray-700">{source.contractId || '—'}</p>
                          </div>
                          <div className="bg-gray-100/80 border border-gray-200 rounded-lg p-3">
                            <p className="text-[10px] font-bold uppercase tracking-widest text-gray-500 mb-1">Content Hash</p>
                            <p className="text-xs font-mono text-gray-700 truncate">{dv.contentHash}</p>
                          </div>
                          <div className="bg-gray-100/80 border border-gray-200 rounded-lg p-3">
                          ...
                          <div className="bg-gray-100/80 border border-gray-200 rounded-lg p-3">
                            ...
                            <p className="text-xs text-gray-600">{source.refreshPolicy}</p>
                          </div>
                        </div>

                        {/* Source description */}
                        <div className="mt-3 p-3 bg-gray-100/80 border border-gray-200 rounded-lg">
            ...
            <div className="flex items-center gap-3 mb-5">
              <div className="relative flex-1 max-w-md">
                ...
                <input
                  ...
                  className="w-full pl-9 pr-4 py-2 bg-gray-100 border border-gray-200 rounded-lg text-xs text-gray-900 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-[#2563EB]/40 focus:border-[#2563EB]/40"
                />
              </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
              {filteredVariables.map(v => {
                ...
                return (
                  <div
                    key={v.id}
                    className="bg-gray-100/80 border border-gray-200 rounded-xl overflow-hidden"
                  >
                    <div
                      className="px-4 py-3.5 cursor-pointer hover:bg-gray-200/40 transition-colors"
                      onClick={() => setExpandedVar(isExpanded ? null : v.id)}
                    >
                      <div className="flex items-start justify-between mb-2">
                        <div className="min-w-0 flex-1">
                          <p className="text-xs font-mono font-bold text-[#2563EB]">{v.varId}</p>
                          <p className="text-sm font-semibold text-gray-900 mt-0.5">{v.label}</p>
                        </div>
                        <ChevronDown className={`h-3.5 w-3.5 text-gray-500 shrink-0 mt-1 transition-transform ${isExpanded ? 'rotate-180' : ''}`} />
                      </div>
                      ...
                      <div className="flex flex-wrap gap-2 mt-2">
                        ...
                        {/* Missingness */}
                        <span className={`text-[10px] font-semibold px-2 py-0.5 rounded-full ${avgMissingness < 0.05 ? 'bg-emerald-900/30 text-emerald-400' : avgMissingness < 0.2 ? 'bg-amber-900/30 text-amber-600' : 'bg-red-900/30 text-red-400'}`}>
                          {(avgMissingness * 100).toFixed(1)}% missing
                        </span>
                        {/* Lock status */}
                        <span className={`text-[10px] font-semibold px-2 py-0.5 rounded-full flex items-center gap-1 ${v.lockStatus === 'locked' ? 'bg-emerald-900/30 text-emerald-400' : 'bg-amber-900/30 text-amber-600'}`}>
                    ...
                    {isExpanded && (
                      <div className="border-t border-gray-200 px-4 py-4 bg-gray-50">
                          ...
                          {v.derivationSteps.map((step, idx) => (
                            <div key={idx} className="flex items-start gap-3">
                              <div className="flex flex-col items-center">
                                <div className="w-5 h-5 rounded-full bg-[#2563EB]/20 border border-[#2563EB]/30 flex items-center justify-center text-[9px] font-bold text-[#2563EB]">{step.order}</div>
                                {idx < v.derivationSteps.length - 1 && <div className="w-px h-6 bg-gray-300 mt-1" />}
                              </div>
                        ...
                        <div className="mt-4 p-3 bg-gray-100/80 border border-gray-200 rounded-lg">
                          <p className="text-[10px] font-bold uppercase tracking-widest text-gray-500 mb-1">Operational Definition</p>
                          <p className="text-[11px] text-gray-600 leading-relaxed">{v.operationalDefinition}</p>
                        </div>
                        ...
                        {v.validationRefs.length > 0 && (
                          <div className="mt-3 flex items-center gap-2 flex-wrap">
                            ...
                            {v.validationRefs.map((ref, i) => (
                              <span key={i} className="text-[10px] text-gray-500 bg-gray-200 px-2 py-0.5 rounded">{ref}</span>
                            ))}
                          </div>
                        )}
          ...
          <div className="space-y-6">
            ...
            <div className="bg-gray-100/80 border border-gray-200 rounded-xl overflow-hidden">
              <div className="px-5 py-4 border-b border-gray-200">
                ...
                <p className="text-sm font-bold text-gray-900 mt-0.5">Field-Level Completeness Across All Sources</p>
              </div>
              <div className="px-5 py-4">
                  ...
                  {(() => {
                    ...
                    return fieldNames.slice(0, 15).map(fname => (
                      <React.Fragment key={fname}>
                        <div className="font-mono text-gray-700 py-1.5 truncate">{fname}</div>
                        {dataSources.map(ds => {
                          const dv = datasetVersions.find(d => d.dataSourceId === ds.id)
                          const field = dv?.fields.find(f => f.name === fname)
                          if (!field) {
                            return <div key={ds.id} className="flex items-center justify-center py-1.5"><span className="w-full h-5 rounded bg-gray-200 flex items-center justify-center text-[9px] text-gray-500">N/A</span></div>
                          }
                          const pct = field.missingnessRate * 100
                          const bg = pct === 0 ? 'bg-emerald-500/20 text-emerald-400' : pct < 5 ? 'bg-emerald-500/30 text-emerald-400' : pct < 10 ? 'bg-amber-500/20 text-amber-600' : pct < 20 ? 'bg-amber-500/30 text-amber-600' : 'bg-red-500/30 text-red-400'
                ...
                <div className="flex items-center gap-4 mt-4 pt-3 border-t border-gray-200">
                  ...
                  <div className="flex items-center gap-1.5"><div className="w-3 h-3 rounded bg-gray-200" /><span className="text-[10px] text-gray-500">N/A</span></div>
                </div>
              </div>
            </div>
            ...
            <div className="bg-gray-100/80 border border-gray-200 rounded-xl overflow-hidden">
              <div className="px-5 py-4 border-b border-gray-200">
                ...
                <p className="text-sm font-bold text-gray-900 mt-0.5">Duplicate & Overlap Validation</p>
              </div>
              <div className="divide-y divide-gray-200">
                {DUPLICATE_CHECKS.map((item, idx) => (
                  <div key={idx} className="flex items-center gap-4 px-5 py-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2">
                        <p className="text-xs font-semibold text-gray-900">{item.check}</p>
                    ...
                    <span className={`text-[10px] font-bold uppercase px-2 py-0.5 rounded-full ${
                      item.result === 'pass' ? 'bg-emerald-900/30 text-emerald-400' : 'bg-amber-900/30 text-amber-600'
                    }`}>
                      {item.result}
                    </span>
                  </div>
            ...
            <div className="bg-gray-100/80 border border-gray-200 rounded-xl overflow-hidden">
              <div className="px-5 py-4 border-b border-gray-200">
                ...
                <p className="text-sm font-bold text-gray-900 mt-0.5">Version Timeline & Currency</p>
              </div>
              <div className="px-5 py-4">
                ...
                {CODING_TIMELINE.map((ct, idx) => (
                  <div key={idx} className="grid grid-cols-[1fr_0.8fr_1fr_1fr_0.6fr] gap-3 py-2 border-t border-gray-200 items-center">
                    <span className="text-xs font-semibold text-gray-900">{ct.system}</span>
                    <span className="text-xs font-mono text-gray-600">{ct.version}</span>
                    ...
                    <span className={`text-[10px] font-semibold px-2 py-0.5 rounded-full text-center ${
                      ct.status === 'active' ? 'bg-emerald-900/30 text-emerald-400' : 'bg-gray-200 text-gray-500'
                    }`}>
                      {ct.status}
                    </span>
                  </div>
            ...
            <div className="bg-gray-100/80 border border-gray-200 rounded-xl overflow-hidden">
              <div className="px-5 py-4 border-b border-gray-200">
                ...
                <p className="text-sm font-bold text-gray-900 mt-0.5">Record Count Trends by Source</p>
              </div>
              <div className="px-5 py-4">
                ...
                {RECORD_TRENDS.map((t, idx) => (
                  <div key={idx} className="grid grid-cols-[100px_repeat(3,1fr)] gap-3 py-2 border-t border-gray-200 items-center">
                    <span className="text-xs font-semibold text-gray-700">{t.period}</span>
        ...
        {activeTab === 'provenance' && (
          <div className="space-y-6">
            <div className="bg-gray-100/80 border border-gray-200 rounded-xl overflow-hidden">
              <div className="px-5 py-4 border-b border-gray-200">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-sm font-bold text-gray-900 mt-0.5">Record Origin Pointers & Linkage Tokens</p>
                  </div>
              ...
              <div className="grid grid-cols-[1fr_0.8fr_1.5fr_1fr_0.8fr_0.5fr] gap-3 px-5 py-2.5 bg-gray-200/60 text-[10px] font-bold uppercase tracking-widest text-gray-500">
                <span>Record ID</span>
              ...
              {PROVENANCE_RECORDS.map((rec, idx) => (
                <div
                  key={idx}
                  className={`grid grid-cols-[1fr_0.8fr_1.5fr_1fr_0.8fr_0.5fr] gap-3 px-5 py-3 border-t border-gray-200 items-center ${idx % 2 === 0 ? '' : 'bg-gray-50'}`}
                >
                  <span className="text-xs font-mono font-semibold text-gray-900">{rec.recordId}</span>
                  ...
                  <span className="text-[11px] text-gray-600 truncate">{rec.source}</span>
                  <div className="flex items-center gap-1.5">
                    <Fingerprint className="h-3 w-3 text-[#2563EB]" />
                    <span className="text-xs font-mono text-[#2563EB]">{rec.linkageToken}</span>
                  </div>
                  ...
                  <div className="flex items-center justify-center">
                    {rec.modifications === 0 ? (
                      ...
                      <span className="text-[10px] font-semibold px-2 py-0.5 rounded-full bg-amber-900/30 text-amber-600">{rec.modifications}</span>
                    )}
                  </div>
                </div>
              ))}
            ...
            {reviewerMode && (
              <div className="bg-[#2563EB]/10 border border-[#2563EB]/30 rounded-xl p-5">
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <p className="text-xs font-bold text-[#2563EB]">Reviewer Note — Provenance Integrity</p>
                </div>
