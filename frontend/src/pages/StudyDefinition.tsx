        ...
        <div className="flex items-start justify-between">
          <div className="flex items-center gap-3">
            <div className="w-8 h-8 rounded-lg bg-[#2563EB]/20 border border-[#2563EB]/30 flex items-center justify-center">
              <FlaskConical className="h-4 w-4 text-[#60a5fa]" />
            </div>
            <div>
      ...
      <div className="px-8 py-6 space-y-6 max-w-4xl">
        ...
        {reviewerMode && (
          <div className="flex items-start gap-3 p-4 bg-[#2563EB]/10 border border-[#2563EB]/30 rounded-xl">
            ...
            <div>
              <p className="text-sm font-semibold text-[#60a5fa]">FDA Reviewer Mode Active</p>
              <p className="text-xs text-gray-400 mt-0.5">Displaying pre-specified protocol elements only. All editable fields are hidden. Rationale and justifications are foregrounded.</p>
            </div>
          </div>
        )}
        ...
        {/* Indication */}
        <section>
          <label className="block text-xs font-bold text-gray-400 uppercase tracking-widest mb-2">Indication</label>
          {locked || reviewerMode ? (
            ...
            <input
              className="w-full bg-gray-100/80 dark:bg-white/4 border border-gray-200 dark:border-white/10 rounded-lg px-4 py-3 text-sm text-gray-900 dark:text-white placeholder-gray-600 focus:outline-none focus:border-[#2563EB]/60 focus:bg-gray-100 dark:focus:bg-white/6 transition-colors"
              value={indication}
        ...
        {/* Primary endpoint */}
        <section>
          <label className="block text-xs font-bold text-gray-400 uppercase tracking-widest mb-2">Primary Endpoint</label>
          {locked || reviewerMode ? (
            ...
            >
              {ENDPOINT_OPTIONS.map(o => <option key={o} value={o} className="bg-[#1A1A1B]">{o}</option>)}
            </select>
          )}
        </section>
        ...
        <section>
          <div className="flex items-center gap-2 mb-2">
            <label className="text-xs font-bold text-gray-400 uppercase tracking-widest">Estimand (ICH E9(R1))</label>
              ...
              <button
                ...
                className={`w-full text-left px-4 py-3 rounded-lg border transition-all ${
                  estimand === opt.value
                    ...
                    : 'bg-white/3 border-gray-200 dark:border-white/8 text-gray-400 hover:bg-gray-100 dark:hover:bg-white/5 hover:text-gray-300'
                } ${locked || reviewerMode ? 'cursor-default' : 'cursor-pointer'}`}
              >
                <div className="flex items-center justify-between">
                  <span className="text-sm font-semibold">{opt.label}</span>
                  {estimand === opt.value && <CheckCircle2 className="h-4 w-4 text-[#60a5fa] shrink-0" />}
                </div>
        ...
        {/* Comparator arm */}
        <section>
          <label className="block text-xs font-bold text-gray-400 uppercase tracking-widest mb-2">Comparator Arm</label>
          {locked || reviewerMode ? (
            ...
            >
              ...
              ].map(o => <option key={o} value={o} className="bg-[#1A1A1B]">{o}</option>)}
            </select>
          )}
        </section>
        ...
        <section>
          ...
          {locked || reviewerMode ? (
            <div className="bg-gray-100/80 dark:bg-white/4 border border-gray-200 dark:border-white/8 rounded-lg px-4 py-3 text-sm text-gray-200 leading-relaxed whitespace-pre-wrap">
              {rationale || <span className="text-gray-600 italic">No rationale entered.</span>}
            </div>
          ) : (
            <textarea
              rows={5}
              className="w-full bg-gray-100/80 dark:bg-white/4 border border-gray-200 dark:border-white/10 rounded-lg px-4 py-3 text-sm text-gray-900 dark:text-white placeholder-gray-600 focus:outline-none focus:border-[#2563EB]/60 focus:bg-gray-100 dark:focus:bg-white/6 transition-colors resize-none"
              value={rationale}
                ...
                <div className="space-y-1.5">
                  {[
                    ...
                    <div key={i} className="flex items-center gap-3 text-xs bg-gray-200/50 dark:bg-white/3 rounded-lg px-3 py-2">
                      ...
                      <span className="text-[#60a5fa] font-semibold w-28 shrink-0">{ice.strategy}</span>
        ...
        {!locked && !reviewerMode && (
          <div className="grid grid-cols-2 gap-4">
            <section>
              <label className="block text-xs font-bold text-gray-400 uppercase tracking-widest mb-2">Regulatory Phase</label>
              <select
                className="w-full bg-gray-100/80 dark:bg-white/4 border border-gray-200 dark:border-white/10 rounded-lg px-4 py-3 text-sm text-gray-900 dark:text-white focus:outline-none focus:border-[#2563EB]/60 transition-colors"
                value={phase} onChange={e => setPhase(e.target.value)}
              >
                {PHASE_OPTIONS.map(o => <option key={o} value={o} className="bg-[#1A1A1B]">{o}</option>)}
              </select>
            </section>
            <section>
              <label className="block text-xs font-bold text-gray-400 uppercase tracking-widest mb-2">Target Regulatory Agency</label>
              <select
                className="w-full bg-gray-100/80 dark:bg-white/4 border border-gray-200 dark:border-white/10 rounded-lg px-4 py-3 text-sm text-gray-900 dark:text-white focus:outline-none focus:border-[#2563EB]/60 transition-colors"
                value={regBody} onChange={e => setRegBody(e.target.value)}
              >
                {REGULATORY_OPTIONS.map(o => <option key={o} value={o} className="bg-[#1A1A1B]">{o}</option>)}
              </select>
            </section>
          </div>
        )}
