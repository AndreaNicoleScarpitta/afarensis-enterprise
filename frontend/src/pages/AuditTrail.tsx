    ...
    <div className="min-h-screen bg-gray-50 text-gray-900">
      ...
      <div className="px-8 py-6 space-y-5 max-w-4xl">
            ...
            <div>
              ...
              <p className="text-xs text-gray-500 mt-0.5">
                ...
                {auditData?.protocol_lock_hash ? (
                  <> Hash: <span className="font-mono text-gray-600">{auditData.protocol_lock_hash}</span> <span className="text-amber-500 text-[10px] font-semibold ml-1">(not independently verified)</span></>
                ) : (
            ...
            <button
              ...
              className={`text-xs font-semibold px-3 py-1.5 rounded-full border transition-colors capitalize ${
                filter === f
                  ...
                  : 'bg-gray-100/80 border-gray-200 text-gray-500 hover:text-gray-600 hover:bg-gray-100'
              }`}
            >
        ...
        <section>
          <div className="border border-gray-200 rounded-xl overflow-hidden">
            {events.map((ev, i) => (
              <div
                key={i}
                className={`flex items-start gap-4 px-4 py-3.5 ${i < events.length - 1 ? 'border-b border-gray-200' : ''} hover:bg-gray-50 transition-colors ${ev.category === 'lock' ? 'bg-emerald-900/10' : ''}`}
              >
                {/* Category badge */}
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm text-gray-900 font-medium leading-snug">{ev.action}</p>
        ...
        <div className="flex items-center justify-between pt-4 border-t border-gray-200">
          <Link to={`/projects/${selectedStudy.id}/reproducibility`} className="flex items-center gap-2 text-gray-500 hover:text-gray-700 text-sm font-medium transition-colors">
            <ChevronLeft className="h-4 w-4" /> Step 8: Reproducibility
          </Link>
          <Link to={`/projects/${selectedStudy.id}/regulatory-output`} className="flex items-center gap-2 bg-[#2563EB] hover:bg-blue-600 text-gray-900 text-sm font-semibold px-5 py-2.5 rounded-lg transition-colors">
