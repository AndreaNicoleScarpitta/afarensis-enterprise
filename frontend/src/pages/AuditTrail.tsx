...
import { ClipboardList, Lock, Eye, ChevronRight, ChevronLeft, CheckCircle2, Shield, Activity } from 'lucide-react'
...
export default function AuditTrail({ selectedStudy, protocolLocked, reviewerMode }: Props) {
  ...
  const { data: auditData, loading, error } = useStudyData(selectedStudy?.id, 'audit')
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-[#0d0d0e] text-gray-900 dark:text-white">
      ...
      <div className="px-8 py-6 space-y-5 max-w-4xl">
        ...
        <div className="flex items-center justify-between pt-4 border-t border-gray-200 dark:border-white/8">
          <a href={`/projects/${selectedStudy.id}/reproducibility`} className="flex items-center gap-2 text-gray-500 hover:text-gray-300 text-sm font-medium transition-colors">
            <ChevronLeft className="h-4 w-4" /> Step 8: Reproducibility
          </a>
          <a href={`/projects/${selectedStudy.id}/regulatory-output`} className="flex items-center gap-2 bg-[#2563EB] hover:bg-blue-600 text-gray-900 dark:text-white text-sm font-semibold px-5 py-2.5 rounded-lg transition-colors">
